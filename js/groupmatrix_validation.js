...
(function($) {
  Drupal.behaviors.ica_event_cmrf_connector = {
    attach: function(context, settings) {
      ...
      if ($(context).is('form#webform-client-form-3') || context === document) {
        // SELECT select[name='submitted[group_registration_table][*][<partner>]']
        $("select[name^='submitted[group_registration_table][']", context)
          .filter(function() {return this.name.endsWith("][" + partner + "]");})
          .on('change', validatePartner)
          .trigger('change');
      }
    }
  };
